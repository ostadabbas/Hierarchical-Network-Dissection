...
class QuantileVector:
    ...
    def add(self, incoming):
        ...
        #print(self.buffersize, self.samplerate, chunksize)
    ...
    def _add_every(self, incoming):
        ...
        while index < supplied:
            ff = self.firstfree[0] # 0
            available = self.data[0].shape[1] - ff # resolution - 0
            if available == 0:
                if not self._shift():
            ...
            self.data[0][:,ff:ff + copycount] = numpy.transpose(incoming[index:index + copycount,:])
            self.firstfree[0] += copycount
            index += copycount
            #print('ff:',self.firstfree, 'data:',[x.shape for x in self.data])
    ...
    def _shift(self):
        ...
        while self.data[index].shape[1] - self.firstfree[index] < (-(-self.data[index-1].shape[1] // 2) if index else 1):
